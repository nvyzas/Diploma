...
bool SkinningTechnique::Init()
{
    ...
    if (!Finalize()) {
		printf("Cannot finalize skinning shaders\n");
        return false;
    }
    ...
    for (unsigned int i = 0 ; i < ARRAY_SIZE_IN_ELEMENTS(m_pointLightsLocation) ; i++) {
        ...
        if (m_pointLightsLocation[i].Color == INVALID_UNIFORM_LOCATION ||
			...
			printf("Invalid uniform location(pointLights)\n");
            return false;
        }
    }
    ...
    for (unsigned int i = 0 ; i < ARRAY_SIZE_IN_ELEMENTS(m_spotLightsLocation) ; i++) {
        ...
        if (m_spotLightsLocation[i].Color == INVALID_UNIFORM_LOCATION ||
			...
			printf("Invalid uniform location(spotLights)\n");
            return false;
        }
    }
...
void SkinningTechnique::SetPointLights(uint NumLights, const PointLight* pLights)
{
...
void SkinningTechnique::SetSpotLights(uint NumLights, const SpotLight* pLights)
{
...
void SkinningTechnique::setBoneTransform(uint index, const QMatrix4x4& transform)
{
    assert(index < MAX_BONES);
    glUniformMatrix4fv(m_boneLocation[index], 1, GL_TRUE, transform.transposed().data());       
}
